...
model PWR_simple_partial_001
  ...
  inner Modelica.Fluid.System system annotation(
    Placement(visible = true, transformation(origin = {-96, 174}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  HeatTransferComponents.ForcedConvection.HTflatPlateTurbSmooth00 hTflatPlate(Amech_par = 0.1, AreaHT_par = AreaHT, Len_par = 5, redeclare package Medium = Modelica.Media.Water.StandardWater, khconv = khconv) annotation(
  ...
  Modelica.Blocks.Sources.Ramp ramp_heat_reactor(duration = 1, height = 1e6, offset = 165*1e6, startTime = 20) annotation(
    Placement(visible = true, transformation(origin = {-48, 122}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  Modelica.Blocks.Interaction.Show.RealValue real_p_reactor_out1(significantDigits = 6, use_numberPort = true) annotation(
    Placement(visible = true, transformation(origin = {30, 16}, extent = {{13, -8}, {-13, 8}}, rotation = 0)));
  Modelica.Blocks.Interaction.Show.RealValue real_AreaHT(number = AreaHT, significantDigits = 6, use_numberPort = false) annotation(
    Placement(visible = true, transformation(origin = {18, 94}, extent = {{-13, -8}, {13, 8}}, rotation = 0)));
  ...
  Modelica.Blocks.Interaction.Show.RealValue real_khconv(number = khconv, significantDigits = 6, use_numberPort = false) annotation(
    Placement(visible = true, transformation(origin = {18, 72}, extent = {{-13, -8}, {13, 8}}, rotation = 0)));
  Modelica.Blocks.Interaction.Show.RealValue real_hconv_reactor_LostCoolant(significantDigits = 4) annotation(
    Placement(visible = true, transformation(origin = {-95, 7}, extent = {{-13, -9}, {13, 9}}, rotation = 0)));
  HeatTransferComponents.Sensors.Temperature_degC_DispColor00 temperature_reactor_LostCoolant(valMax = valMaxTContour, valMin = valMinTContour) annotation(
    Placement(visible = true, transformation(origin = {-90, -33}, extent = {{-12, -6}, {12, 6}}, rotation = -90)));
  HeatTransferComponents.NaturalConvection.HTverticalPlate00 reactor_LostCoolant(AreaHT_par = AreaHT, Lc = hTflatPlate.Len_par, redeclare package Medium = Modelica.Media.Air.DryAirNasa, khconv = khconv) annotation(
    Placement(visible = true, transformation(origin = {-128, -29.6667}, extent = {{20.3334, -20.3333}, {-20.3334, 28.4667}}, rotation = 0)));
  ...
  Modelica.Fluid.Sources.Boundary_pT ambientAir(redeclare package Medium = Modelica.Media.Air.DryAirNasa, T = 288.15, nPorts = 2, p = 100*1000) annotation(
    Placement(visible = true, transformation(origin = {-73, -76}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  FluidSystemComponents.Sensor.Temperature_degC_DispColor00 temperature_ambAir(redeclare package Medium = Modelica.Media.Air.DryAirNasa, valMax = valMaxTContour, valMin = valMinTContour) annotation(
    Placement(visible = true, transformation(origin = {-77, -95}, extent = {{-11, -5}, {11, 5}}, rotation = 0)));
equation
  connect(steam_generator_inlet.ports[1], hTflatPlate.port_2) annotation(
    Line(points = {{112, 62}, {68, 62}, {68, 16}}, color = {0, 127, 255}, thickness = 1));
  ...
  connect(ramp_heat_reactor.y, reactor.Q_flow) annotation(
    Line(points = {{-48, 111}, {-48, -7}, {-42, -7}}, color = {0, 0, 127}));
  connect(real_p_reactor_out1.numberPort, hTflatPlate.y_h) annotation(
    Line(points = {{44.95, 16}, {51.95, 16}}, color = {0, 0, 127}));
  ...
  connect(temperature_reactor_LostCoolant.heatPort, reactor_LostCoolant.heatPort) annotation(
    Line(points = {{-90, -21}, {-102, -21}, {-102, -30}, {-108, -30}}, color = {191, 0, 0}));
  connect(real_hconv_reactor_LostCoolant.numberPort, reactor_LostCoolant.y_h) annotation(
    Line(points = {{-110, 7}, {-110, -1}}, color = {0, 0, 127}));
  connect(reactor_LostCoolant.heatPort, ptHeatSrc_LostCoolant.port) annotation(
    Line(points = {{-108, -30}, {-74, -30}}, color = {191, 0, 0}, thickness = 1));
  connect(ptHeatSrc_LostCoolant.Q_flow, ramp_heat_reactor.y) annotation(
    Line(points = {{-54, -30}, {-48, -30}, {-48, 112}}, color = {0, 0, 127}));
  connect(reactor_LostCoolant.portStat, ambientAir.ports[1]) annotation(
    Line(points = {{-128, -50}, {-128, -76}, {-82, -76}}, color = {0, 127, 255}, thickness = 1));
  connect(ambientAir.ports[2], temperature_ambAir.port) annotation(
    Line(points = {{-82, -76}, {-88, -76}, {-88, -96}}, color = {0, 127, 255}));
  annotation(
    Diagram(coordinateSystem(extent = {{-200, -200}, {200, 200}}), graphics = {Text(origin = {6, 104}, extent = {{-28, 6}, {28, -6}}, textString = "Eff area of heat transfer"), Text(origin = {-3, -39}, extent = {{-13, 3}, {13, -3}}, textString = "[MW]"), Text(origin = {39, 93}, extent = {{-13, 3}, {13, -3}}, textString = "[m2]"), Text(origin = {42, 72}, extent = {{-13, 3}, {13, -3}}, textString = "[nond]"), Text(origin = {26, 81}, extent = {{-48, 9}, {48, -9}}, textString = "Factor on convective heat transfer coefficient"), Line(origin = {120.15, -9.05}, points = {{-17, -32}, {-17, 0}}, thickness = 1.5, arrow = {Arrow.None, Arrow.Open})}),
